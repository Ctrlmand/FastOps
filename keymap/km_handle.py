...
from ..function.debug import LogRow
...
def register():
    ...
    kmi_image_editor_channel_red = km_image_editor.keymap_items.new(operators.image_editor.F_OT_ImageChannelSet.bl_idname, 'R', 'PRESS', ctrl=False, shift=False)
    kmi_image_editor_channel_red.properties.channel = 'RED'

    kmi_image_editor_channel_green = km_image_editor.keymap_items.new(operators.image_editor.F_OT_ImageChannelSet.bl_idname, 'G', 'PRESS', ctrl=False, shift=False)
    kmi_image_editor_channel_green.properties.channel = 'GREEN'
    
    kmi_image_editor_channel_blue = km_image_editor.keymap_items.new(operators.image_editor.F_OT_ImageChannelSet.bl_idname, 'B', 'PRESS', ctrl=False, shift=False)
    kmi_image_editor_channel_blue.properties.channel = 'BLUE'

    kmi_image_editor_channel_alpha = km_image_editor.keymap_items.new(operators.image_editor.F_OT_ImageChannelSet.bl_idname, 'A', 'PRESS', ctrl=False, shift=False)
    kmi_image_editor_channel_alpha.properties.channel = 'ALPHA'

    kmi_image_editor_channel_color = km_image_editor.keymap_items.new(operators.image_editor.F_OT_ImageChannelSet.bl_idname, 'C', 'PRESS', ctrl=False, shift=False)
    kmi_image_editor_channel_color.properties.channel = 'COLOR'

    kmi_image_editor_channel_color_alpha = km_image_editor.keymap_items.new(operators.image_editor.F_OT_ImageChannelSet.bl_idname, 'C', 'PRESS', ctrl=True, shift=False)
    ...
    kmi_object_3dview_call_modifier_pie.properties.name = ui.view3D_pie.VIEW3D_MT_ModifierPieMenu.bl_idname
    ...
    kmi_object_3dview_call_material_pie.properties.name = ui.view3D_pie.VIEW3D_MT_AlternatePieMenu.bl_idname
    ## append to km
    ...
    # >> debug << 
    LogRow(colorCode= 96, info = ">> KM REGISTED <<", separator='-', separator_col=96, side="-")
    # traverse km and kmi
    for km, kmi_list in addon_km:
        LogRow(colorCode= 36, info = f"> {km.name} <", separator='', separator_col=96, side="|")
        for kmi in kmi_list:
            LogRow(colorCode= 32, info = f"{kmi.name}", separator='', separator_col=96, side="|", left=True)
    # end
    LogRow(colorCode=96, info=f"", separator="-",separator_col=96, side= "-")
# Unregister
def unregister():
    for km, kmi_list in addon_km:
        for kmi in kmi_list:
